...
import { RatingBadge, PriceDisplay, formatPrice } from '@steam-clone/ui'
  ...
  return (
    <div className="relative h-[240px] overflow-hidden rounded-sm group">
      ...
      <div className="absolute inset-0 p-5 flex flex-col justify-end w-[45%]">
        ...
        <RatingBadge summary={active.rating.summary} className="text-[11px]" />
        <div className="mt-1">
          <PriceDisplay price={active.price} size="sm" />
        </div>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 flex">
        {featured.map((g, i) => (
          <Button
            ...
            variant="ghost"
            onClick={() => setIdx(i)}
            className={cn('flex-1 relative h-[48px] overflow-hidden p-0 rounded-none transition-opacity', idx === i ? 'opacity-100' : 'opacity-50 hover:opacity-75')}
          >
        ...
        <ScrollArea>
          <div className="flex gap-2 pb-2">
            {isLoading
              ...
              : featured?.map(g => <GameCardLocal key={g.id} game={g} />)
            }
          </div>
